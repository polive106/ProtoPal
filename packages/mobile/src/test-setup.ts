...
// Mock expo-localization — default to English device locale
vi.mock('expo-localization', () => ({
  getLocales: () => [{ languageCode: 'en', languageTag: 'en-US' }],
}));

// Global mock for @acme/i18n — resolves translation keys to English strings
// so that text-based test assertions work after i18n migration.
vi.mock('@acme/i18n', () => {
  const commonEn = {
    appName: 'ProtoPal',
  ...
  return {
    ...
    translateApiError: (error: { errorKey?: string; message: string }) => error.message,
    default: mockI18n,
  };
});
